...
accusation_file = r"acc_list.txt"
accusation_list = json.load(open(accusation_file, "r", encoding='utf8'))
...
def draw_out(in_path, out_path):
    input_data = json.load(open(in_path, "r"))

    cnt = 0
    for data in input_data:
        try:
            data["meta_info"] = parse(data["origin"])
        except Exception as e:
            print(e)
            gg

    json.dump(input_data, open(out_path, "w", encoding="utf8"), indent=2, ensure_ascii=False, sort_keys=True)


if __name__ == "__main__":
    prefix = "" # set your path here

    for filename in ["train.json", "valid.json"]:
        draw_out(os.path.join(prefix, "full." + filename), os.path.join(prefix, filename))
